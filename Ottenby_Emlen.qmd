...
ggplot(aes(x = sector, y = count), data = datalong_EmlenInd)+
    ...
    geom_segment(data=mergedEmlen,
               mapping=aes(y = 0, x = Mean, xend = Mean, 
                           yend = (test_statistic*max(datalong_EmlenInd$count))),
...
```{r AllInd_sp}
#GUARDAR EN PDF PARA VERLO BONITO
...
```{r Circular_ANOVA}
aov.circular(EmlenData_ForTest$Mean, EmlenData_ForTest$Treatment, kappa = NULL,
...
Emlen_capture <- Emlen_capture %>%
  ...
  setnames(old = c("test_statistic", "p_value", "Mean", 
                  "Lower_CI", "Upper_CI"),
...
```{r Angular_diff}
Angular_diff_CC <- (Emlen_hotelling$Mean_control-Emlen_hotelling$Mean_capture)
Angular_diff_CC <- adjust_360(Angular_diff_CC)
Angular_diff_CC <- as.circular(Angular_diff_CC,units='degrees', template='geographics', modulo="2pi")

Angular_diff_CT <- (Emlen_hotelling$Mean_treatment-Emlen_hotelling$Mean_control)
Angular_diff_CT <- adjust_360(Angular_diff_CT)
Angular_diff_CT <- as.circular(Angular_diff_CT,units='degrees', template='geographics', modulo="2pi")

Angular_diff_CapT <- (Emlen_hotelling$Mean_treatment-Emlen_hotelling$Mean_capture)
Angular_diff_CapT <- adjust_360(Angular_diff_CapT)
Angular_diff_CapT <- as.circular(Angular_diff_CapT,units='degrees', template='geographics', modulo="2pi")
```
...
```{r Angular_plot}
plot(Angular_diff_CC, main = "Angular change capture vs control", points.plot=TRUE, xlim=c(-1.5,1), ylim=c(-1.1, 1.5))
arrows.circular(mean.circular(Angular_diff_CC), length=0.1)

plot(Angular_diff_CT, main = "Angular change control vs treatment", points.plot=TRUE, xlim=c(-1.5,1), ylim=c(-1.1, 1.5))
arrows.circular(mean.circular(Angular_diff_CT), length=0.1)

plot(Angular_diff_CapT, main = "Angular change capture vs treatment", points.plot=TRUE, xlim=c(-1.5,1), ylim=c(-1.1, 1.5))
arrows.circular(mean.circular(Angular_diff_CapT), length=0.1)
```
...
#emmeans(Emlen_lmer_Ring, list(pairwise ~ radians:Treatment), adjust="tukey")
