...
```{r}
less_alpha<-subset(mergedEmlen, p_value>0.06)
...
```{r}
mergedEmlen[!(mergedEmlen$Identificator %in% less_alpha$Identificator),]
```
...
EmlenData_ForTest<- filter(EmlenData_ForTest, 
                           Species == c("Robin","Willow_Warbler"))
...
ggplot() +
  ...
  geom_line(data = data.frame(emm_DS), 
            aes(x = radians, y = FUN_RevTrans_pez(emmean)), linewidth = 1)+
  facet_wrap(Treatment~BreedingDistribution)
