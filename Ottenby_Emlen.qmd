...
This document has the codes and figures generated for Maria Laura's Master thesis - Orientation analysis for spring season 2023.
...
```{r database, message = FALSE, warning=FALSE}
#| output: false
#| include: false
...
```{r newSectorNames, message = FALSE, warning=FALSE}
...
```{r DirectionTable, message = FALSE, warning=FALSE}
...
```{r DegRad_Functions, message = FALSE, warning=FALSE}
...
Cleaning this code I realized that I don't know how I thought about this. I can't think right now on how to do functions or search for them (I am quite sure this is not my creation, but I had the idea that it could be useful). Lost train of thinking I guess.

```{r adjust360_fun, message = FALSE, warning=FALSE}
...
```{r Weeks, message = FALSE, warning=FALSE}
...
```{r data_long, message = FALSE, warning=FALSE}
...
```{r count_prop, message = FALSE, warning=FALSE}
...
```{r uniqueIdent, message = FALSE, warning=FALSE}
...
```{r Results_unimodal, message = FALSE, warning=FALSE}
#| warning: false
...
```{r combined_df, message = FALSE, warning = FALSE}
combined_df <- bind_rows(results_df_unimodal, results_df_axial, .id = "Table")
```

And now we can have the table with the distribution for each bird

```{r results_df, message = FALSE, warning = FALSE}
results_df <- combined_df %>%
  group_by(Identificator) %>%
  slice(which.max(rho)) #Keep the values that are associated with the highest rho val
results_df <- dplyr::rename(results_df, Distribution = Table) #Change the column name
...
```{r results360adj, message = FALSE, warning = FALSE}
#| output: false
#| include: false
...
```{r mergedEmlen, message = FALSE, warning = FALSE}
...
```{r uniqueValues_Ring, message = FALSE, warning = FALSE}
...
```{r occurrences, message = FALSE, warning = FALSE}
occurrences <- count(mergedEmlen$Ring) 
occurrences <- dplyr::rename(occurrences, Ring = x)
occurrences <- dplyr::rename(occurrences, n = freq)
print(occurrences)
```
...
```{r Results_treat, message = FALSE, warning=FALSE}
#| warning: false
#| output: false
#| include: false
...
Results_treat_all_unimodal <- EmlenData_sp_Mean %>%
  ...
  dplyr::summarize(
    N=length(Mean),
    Lower_CI = if (dplyr::n() > 1) confidence_interval(Mean, conf.level = 0.95,
                                                axial = FALSE)$conf.interval[1] 
    else NA,
    Upper_CI = if (dplyr::n() > 1) confidence_interval(Mean, conf.level = 0.95,
...
Results_treat_all_axial <- EmlenData_sp_Mean %>%
  ...
  dplyr::summarize(
    N=length(Mean),
    Lower_CI = if (dplyr::n() > 1) ((confidence_interval((Mean*2), conf.level = 0.95,
                                                axial = FALSE)$conf.interval[1])/2)+180 
    else NA,
    Upper_CI = if (dplyr::n() > 1) ((confidence_interval((Mean*2), conf.level = 0.95,
...
Results_treat_all <- dplyr::rename(Results_treat_all, Distribution = Table) #Change the column name
