...
When we load the workspace th database that we just produced should be available

```{r RWorkspace, message = FALSE, warning = FALSE}
...
```{r plot_raw, message = FALSE, warning = FALSE}
#Code made by Giuseppe Bianco
...
Now I can set up the data I will work with from now in this code.

Let's set the db to work with

```{r stf_data, message = FALSE, warning = FALSE}
stf_data<-db.act
```

We can extract time as a separate column

```{r stf_time, message = FALSE, warning = FALSE}
...
The first random effect lineal model is the activity by Day or night per setup (contrl vs treatment) per day, having as a random effect the individual (ring)
...
This are the p-values for each pair of days and day/night. These p-values are to table

```{r pairs_table_mod1, message = FALSE, warning = FALSE}
...
```{r db_mod1, message = FALSE, warning = FALSE}
...
```{r plot_mod1, message = FALSE, warning = FALSE}
#| warning: false 
...
Lets chech the r2
rcond: Represents the proportion of variance explained by the entire model, including both fixed effects and random effects.
rmarg: Represents the proportion of variance explained solely by the fixed effects

```{r perform_mod1, message = FALSE, warning = FALSE}
...
The second random effect lineal model is the total activity with the interaction between the experiment setup (control vs treatment) per day
...
```{r check_mod3, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
check_model(stf_mod_Exp_meanTotal)
```

And the summary of the model

```{r summary_mod3, message = FALSE, warning = FALSE}
summary(stf_mod_Exp_meanTotal)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod3, message = FALSE, warning = FALSE}
...
```{r pairs_mod3, message = FALSE, warning = FALSE}
pairs(stf_total_mod3)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod3, message = FALSE, warning = FALSE}
...
Here, again, is the table with the whole comparison between days for the model

```{r pairs_table_mod3, message = FALSE, warning = FALSE}
...
```{r db_mod3, message = FALSE, warning = FALSE}
...
```{r plot_mod3, message = FALSE, warning = FALSE}
#| warning: false   
...
```{r perform_mod3, message = FALSE, warning = FALSE}
...
Comparison null model and model 1 (act Day/Night per setup, Day or night per day)

```{r null_mod1, message = FALSE, warning = FALSE}
...
```
Comparison null model and model 3 (actTotal per setup per day)

```{r null_mod3, message = FALSE, warning = FALSE}
...
This actograms did not make it to the paper. I have this code here because I wanted to produce them during the thesis. Will it be useful in the furture? I don't know. I never know.
...
```{r stf_actogram_raw, message = FALSE, warning = FALSE}
...
```{r actogram_Ring, message = FALSE, warning = FALSE}
