...
#
pdf("plot_ind_Species_test11.pdf", onefile=TRUE)
...
for(i in species_unique) {
  ...
  p1<-ggplot(aes(x = sector, y = count), data = dat_count)+
    ...
    facet_grid(Treatment~Ring)+
    theme_minimal()
 print(p1)
}
dev.off()

pdf("plot_ind_Species_test22.pdf", height=45, width=30, onefile=TRUE)
...
#Plot individuals that meet the criteria
ggplot(aes(x = Mean, y=y), data=stf_Emlen_pre) +
  geom_point(size = 1.8, alpha=0.5, shape=21, stroke = 0.5, colour = "black",
             aes(fill=factor(Treatment)))+
  coord_polar() +
  scale_x_continuous("", limits=c(0,360),
...
```{r}
Moore_Control<- MooreRStats(deg2rad(Emlen_hotelling_C$Mean_pre), 
                       deg2rad(Emlen_hotelling_C$Mean_post)) 
Moore_Treatment<-MooreRStats(deg2rad(Emlen_hotelling_T$Mean_pre), 
                        deg2rad(Emlen_hotelling_T$Mean_post))


cosphi_Control <- Moore_Control[[1]] ; sinphi_Control <- Moore_Control[[2]]
Ranks_Control <- Moore_Control[[3]]
cosphi_Treatment <- Moore_Treatment[[1]] ; sinphi_Treatment <- Moore_Treatment[[2]]
Ranks_Treatment <- Moore_Treatment[[3]]

MooreRTestRand(cosphi_Control, sinphi_Control, Ranks_Control, 9999)
MooreRTestRand(cosphi_Treatment, sinphi_Treatment, Ranks_Treatment, 9999)
```
