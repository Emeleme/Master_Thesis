...
```{r}
stf_combined_df <- bind_rows(stf_results_df_unimodal, stf_results_df_axial, .id = "Table")
```
...
stf_Results_treat <- subset(stf_Results_treat, N>12)
...
```{r treat_subset}
stf_Emlen_control <- stf_Emlen_control %>%
...
stf_Emlen_treatment <- stf_Emlen_treatment %>%
  dplyr::select(-c("Treatment", "Identificator", "Species", "BreedingDistribution", "y")) %>%
  setnames(old = c("rho", "p_value", "Mean", 
                  "Lower_CI", "Upper_CI"),
...
```{r common_ids}
stf_common_ids <- Reduce(intersect, list(stf_Emlen_control$Ring, stf_Emlen_treatment$Ring))
```

To retrieve the Rings that have capture, control and treatment experiments

```{r Emlen_ID}
stf_Emlen_control_ID <- stf_Emlen_control[stf_Emlen_control$Ring %in% stf_common_ids, ]
stf_Emlen_treatment_ID <- stf_Emlen_treatment[stf_Emlen_treatment$Ring %in% stf_common_ids, ]
```

And to merge a database for making the Hotelling test

```{r hotelling_db}
stf_Emlen_hotelling <- merge(stf_Emlen_control_ID, stf_Emlen_treatment_ID, by="Ring")
```
...
paired.hotelling(stf_Emlen_hotelling$Mean_control, stf_Emlen_hotelling$Mean_treatment)
```
