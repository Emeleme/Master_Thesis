...
PLEASE NOTE: For the paper that was generated from this experiments some terms 
were changed for comprenhension in the text. But it was very confusing for me to
change the whole text without messing it up a little bit. In this sense please 
have in mind that:

- Day and Night = Light and Dark PHASE
- Control vs treatment = STAGE

Sorry about this. I´ll fix it in future studies.
...
```{r 2h, message = FALSE, warning = FALSE}
...
```{r act_col, message = FALSE, warning = FALSE}
...
```{r Vib-, message = FALSE, warning = FALSE}
...
```{r Cage_name, message = FALSE, warning = FALSE}
...
```{r Cage_name7, message = FALSE, warning = FALSE}
...
```{r Week6, message = FALSE, warning = FALSE}
...
```{r HourDecimal, message = FALSE, warning = FALSE}
...
```{r ControlTreatment, message = FALSE, warning = FALSE}
activity_pivot <- activity_pivot %>%
...
```{r Whyyy, message = FALSE, warning = FALSE}
...
```{r Merge_activity, message = FALSE, warning = FALSE}
...
Lets check how many ring do we have here to control what we just did :) Again, they shoudl be 73.
...
```{r Capture-, message = FALSE, warning = FALSE}
merged_activity <- merged_activity %>% #Delete capture & no trial data
  filter(!experiment %in% c("Capture", "not trial"))
```

Lets check how many ring do we have here. Again, just to control we did not delete any individual (should again be 73)
...
```{r AfterEmlen-, message = FALSE, warning = FALSE}
...
```{r DayNigth, message = FALSE, warning = FALSE}
...
How many individuals do I have in the db. They should be 68, some of them were released and not analyzed in the days that I did not consider for analysis.
...
```{r Feeding-, message = FALSE, warning = FALSE}
...
```{r ExperimentalDay, message = FALSE, warning = FALSE}
...
First lets do the mean activity per Day and Night. Here I check how is the distribution
of the activity throughout the day. 

```{r MeanDayNight, message = FALSE, warning = FALSE}
...
This model is the interaction between the experiment (control vs treatment) during day and night having as a random effect the ring and species.

```{r mod1s, message = FALSE, warning = FALSE}
...
```{r pairs_mod1s, message = FALSE, warning = FALSE}
...
```{r grid_mod1s, message = FALSE, warning = FALSE}
...
```{r db_mod1s, message = FALSE, warning = FALSE}
mean_mod1s<-as.data.frame(mean_mod1s)
raw_Exp_DayNight_MeanData <- mean_activity_DayNight[
  c("DayNight", "experiment", "DayNight_activity", "Species")] #So we can plot things with color per species
...
```{r MeanActivityPerStage_plot, dev = "pdf", message = FALSE, warning = FALSE}
ggplot(mean_mod1s, aes(x = DayNight, y = emmean)) +
  geom_jitter(data = plot_data_mod1s, 
              aes(x = DayNight, y = DayNight_activity, color=Species), 
              alpha = 0.7, show.legend = FALSE) +
  geom_point(position = position_dodge2(width = 2), size =2.8) +
...
```{r mod2s, message = FALSE, warning = FALSE}
...
```{r check_mod2s, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
check_model(mod_ExpBreed_meanDayNight_s)
```

And the summary of the model

```{r summary_mod2s, message = FALSE, warning = FALSE}
summary(mod_ExpBreed_meanDayNight_s)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod2s, message = FALSE, warning = FALSE}
...
```{r pairs_mod2s, message = FALSE, warning = FALSE}
pairs(mean_mod2s, adjust=NULL)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod2s, message = FALSE, warning = FALSE}
...
```{r db_mod2s, message = FALSE, warning = FALSE}
mean_mod2s<-as.data.frame(mean_mod2s)
raw_ExpDayNightBreed_MeanData_s <- mean_activity_DayNight[
  c("DayNight", "experiment", "DayNight_activity", "BreedingDistribution", "Species")]
...
```{r plot_mod2s, message = FALSE, warning = FALSE}
ggplot(mean_mod2s, aes(x = DayNight, y = emmean, shape=experiment)) +
   geom_jitter(data = plot_data_mod2s %>% filter(BreedingDistribution == "Polar"),
              aes(x = DayNight, y = DayNight_activity, shape=experiment, color = Species),
              alpha = 0.7, show.legend = FALSE,
              position = position_dodge2(width = 0.6, padding = 0.5)) +  # Polar raw data
   geom_jitter(data = plot_data_mod2s %>% filter(BreedingDistribution == "Subpolar"),
              aes(x = DayNight, y = DayNight_activity, shape=experiment, color = Species),
              alpha = 0.7, show.legend = FALSE, 
   ...
   geom_point(aes(shape=experiment), 
              position = position_dodge2(width = 0.6, padding = 0.5), size = 2.8) +
...
```{r mod3s, message = FALSE, warning = FALSE}
...
```{r check_mod3s, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
check_model(mod_Exp_meanTotal_s)
```

And the summary of the model

```{r summary_mod3s, message = FALSE, warning = FALSE}
summary(mod_Exp_meanTotal_s)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod3s, message = FALSE, warning = FALSE}
...
```{r pairs_mod3s, message = FALSE, warning = FALSE}
pairs(total_mod3s, adjust=NULL)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod3s, message = FALSE, warning = FALSE}
...
```{r plot_mod3s, message = FALSE, warning = FALSE}
ggplot(total_mod3s, aes(x = experiment, y = emmean)) +   
  geom_jitter(data = plot_data_mod3s, aes(x = experiment, y = DayNight_activity, 
                                          color=Species),alpha = 0.7, show.legend = FALSE)+
  geom_point(position = position_dodge2(width = 2), size =2.8) +   
  geom_linerange(aes(ymin = lower.CL, ymax = upper.CL), alpha = 0.8,              
                 position = position_dodge2(width = 1), size=1)+   
...
```{r mod4s, message = FALSE, warning = FALSE}
...
```{r check_mod4s, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
check_model(mod_ExpBreed_meanTotal_s)
```

And the summary of the model

```{r summary_mod4s, message = FALSE, warning = FALSE}
summary(mod_ExpBreed_meanTotal_s)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod4s, message = FALSE, warning = FALSE}
...
```{r pairs_mod4s, message = FALSE, warning = FALSE}
pairs(total_mod4s, adjust=NULL)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod4s, message = FALSE, warning = FALSE}
#| warning: false   
...
```{r db_mod4s, message = FALSE, warning = FALSE}
total_mod4s<-as.data.frame(total_mod4s)
raw_Exp_DayNight_TotalData <- mean_activity_Total[
  c("experiment", "DayNight_activity", "BreedingDistribution", "Species")]  
...
```{r plot_mod4s, message = FALSE, warning = FALSE}
#| warning: false
...
```{r null_mods, message = FALSE, warning = FALSE}
...
```{r null_Total, message = FALSE, warning = FALSE}
...
```{r r2, message = FALSE, warning = FALSE}
...
```{r null_performance, message = FALSE, warning = FALSE}
...
```{r RobinSubset, message = FALSE, warning = FALSE}
Robins_act<- subset(merged_activity, Species=="Robin")
```

How many Robins do I have? They should be 33

```{r UniqueRobins, message = FALSE, warning = FALSE}
length(unique(Robins_act$Ring)) 
```
...
```{r MeanDayNight_R, message = FALSE, warning = FALSE}
...
```{r mod1_R, message = FALSE, warning = FALSE}
...
```{r check_mod1_R, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
check_model(mod_Exp_meanDayNight_R)
```

And the summary of the model

```{r summary_mod1_R, message = FALSE, warning = FALSE}
summary(mod_Exp_meanDayNight_R)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod1_R, message = FALSE, warning = FALSE}
...
```{r pairs_mod1_R, message = FALSE, warning = FALSE}
pairs(mean_mod1_R, adjust=NULL)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod1_R, message = FALSE, warning = FALSE}
...
```{r db_mod1_R, message = FALSE, warning = FALSE}
...
```{r plot_mod1_R, message = FALSE, warning = FALSE}
#| warning: false
...
```{r r2_mod1_R, message = FALSE, warning = FALSE}
model_performance(mod_Exp_meanDayNight_R)
```

model comparison

```{r null_mod1_R, message = FALSE, warning = FALSE}
# mod_Exp_meanDayNight_R<- lmer(DayNight_activity ~ DayNight * experiment + (1|Ring),
#                               data = Robin_mean_activity_DayNight, na.action = na.exclude)
...
```{r mod2_R, message = FALSE, warning = FALSE}
...
```{r check_mod2_R, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
check_model(mod_Exp_meanTotal_R)
```

And the summary of the model

```{r summary_mod2_R, message = FALSE, warning = FALSE}
summary(mod_Exp_meanTotal_R)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod2_R, message = FALSE, warning = FALSE}
...
```{r pairs_mod2_R, message = FALSE, warning = FALSE}
pairs(mean_mod2_R, adjust=NULL)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod2_R, message = FALSE, warning = FALSE}
...
```{r db_mod2_R, message = FALSE, warning = FALSE}
...
```{r plot_mod2_R, message = FALSE, warning = FALSE}
#| warning: false
...
```{r r2_mod2_R, message = FALSE, warning = FALSE}
model_performance(mod_Exp_meanTotal_R) 
```

model comparison

```{r null_mod2_R, message = FALSE, warning = FALSE}
# mod_Exp_meanTotal_R<- lmer(DayNight_activity ~ experiment + (1|Ring),  
#                            data = Robin_mean_activity_Total, na.action = na.exclude)
...
```{r WillysSubset, message = FALSE, warning = FALSE}
Willy_act<- subset(merged_activity, Species=="Willow_Warbler")
```

How many Willys do I have?

```{r UniqueWillys, message = FALSE, warning = FALSE}
...
```{r MeanDayNight_W, message = FALSE, warning = FALSE}
...
```{r mod1_W, message = FALSE, warning = FALSE}
...
```{r check_mod1_W, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
check_model(mod_Exp_meanDayNight_W)
```

And the summary of the model

```{r summary_mod1_W, message = FALSE, warning = FALSE}
summary(mod_Exp_meanDayNight_W)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod1_W, message = FALSE, warning = FALSE}
...
```{r pairs_mod1_W, message = FALSE, warning = FALSE}
pairs(mean_mod1_W, adjust=NULL)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod1_W, message = FALSE, warning = FALSE}
...
```{r db_mod1_W, message = FALSE, warning = FALSE}
...
```{r plot_mod1_W, message = FALSE, warning = FALSE}
#| warning: false 
...
```{r r2_mod1_W, message = FALSE, warning = FALSE}
model_performance(mod_Exp_meanDayNight_W)
```

model comparison

```{r null_mod1_W, message = FALSE, warning = FALSE}
# mod_Exp_meanDayNight_W<- lmer(DayNight_activity ~ DayNight * experiment + (1|Ring),    
#                               data = Willy_mean_activity_DayNight, na.action = na.exclude)
...
```{r mod2_W, message = FALSE, warning = FALSE}
...
```{r check_mod2_W, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
check_model(mod_Exp_meanTotal_W)
```

And the summary of the model

```{r summary_mod2_W, message = FALSE, warning = FALSE}
summary(mod_Exp_meanTotal_W)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod2_W, message = FALSE, warning = FALSE}
...
```{r pairs_mod2_W, message = FALSE, warning = FALSE}
pairs(mean_mod2_W, adjust=NULL)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod2_W, message = FALSE, warning = FALSE}
...
```{r db_mod2_W, message = FALSE, warning = FALSE}
...
```{r plot_mod2_W, message = FALSE, warning = FALSE}
#| warning: false 
...
```{r r2_mod2_W, message = FALSE, warning = FALSE}
model_performance(mod_Exp_meanTotal_W) 
```

model comparison

```{r null_mod2_W, message = FALSE, warning = FALSE}
# mod_Exp_meanTotal_W<- lmer(DayNight_activity ~ experiment + (1|Ring),     
#                            data = Willy_mean_activity_Total, na.action = na.exclude)
...
This plots are not shown in the paper. This was my academic exercise to try to recreate actograms with my data. The data shown in the paper was generated by one of the authors using other coding methods. Please, refer to those plots in the need of clarification.

All the plots here are produced as a modified version of the function "actogram in the"digiRythm" package

```{r actogram_raw, message = FALSE, warning = FALSE}
...
```{r alias_activity, message = FALSE, warning = FALSE}
...
```{r actogram_datetime, message = FALSE, warning = FALSE}
...
```{r w1_times, message = FALSE, warning = FALSE}
...
```{r dataplot_w1, message = FALSE, warning = FALSE}
...
```{r plot_w1, fig.width=10, fig.height=10, message = FALSE, warning = FALSE}
#| warning: false 
...
```{r w2_times, message = FALSE, warning = FALSE}
...
```{r dataplot_w2, message = FALSE, warning = FALSE}
...
```{r plot_w2, fig.width=10, fig.height=10, message = FALSE, warning = FALSE}
#| warning: false 
...
```{r w3_times, message = FALSE, warning = FALSE}
...
```{r dataplot_w3, message = FALSE, warning = FALSE}
...
```{r plot_w3, fig.width=10, fig.height=10, message = FALSE, warning = FALSE}
#| warning: false 
...
```{r w4_times, message = FALSE, warning = FALSE}
...
```{r dataplot_w4, message = FALSE, warning = FALSE}
...
```{r plot_w4, fig.width=11, fig.height=11, message = FALSE, warning = FALSE}
#| warning: false 
...
```{r w5_times, message = FALSE, warning = FALSE}
...
```{r dataplot_w5, message = FALSE, warning = FALSE}
...
```{r plot_w5, fig.width=11, fig.height=11, message = FALSE, warning = FALSE}
#| warning: false 
...
```{r day_times, message = FALSE, warning = FALSE}
start_day <- "1"
end_day <- "7"
```
...
```{r dataplot_day, message = FALSE, warning = FALSE}
data_to_plot_day <- data_actogram_filter
```

And the actogram can be plotted now

```{r plot_day, message = FALSE, warning = FALSE}
#| warning: false  
...
```{r plot_breeding, message = FALSE, warning = FALSE}
#| warning: false  
ggplot(data_to_plot_day, aes(x = time, y = DayNumber, fill = !!sym(activity))) +
  geom_tile() +
...
```{r Willys-, message = FALSE, warning = FALSE}
#merged_activity <- merged_activity %>%  
#  filter(!Ring %in% c("DK53569"))
```


This model is the interaction between the experiment (control vs treatment) during day and night and taking into account the week of capture but also having as a random effect the species.

```{r mod1s, message = FALSE, warning = FALSE}
#mod_Exp_meanDayNight_Week_s<- lmer(DayNight_activity ~ DayNight * experiment + Week + (1|Ring) + (1|Species), data = mean_activity_DayNight, na.action = na.exclude)
```

To check the model fits some assumptions

```{r check_mod1s, fig.width=7, fig.height=9, message = FALSE, warning = FALSE}
#check_model(mod_Exp_meanDayNight_Week_s)
```

And the summary of the model

```{r summary_mod1s, message = FALSE, warning = FALSE}
#summary(mod_Exp_meanDayNight_Week_s)
```

Lets calculate the means and the confidence intervals for the model

```{r CI_mod1s, message = FALSE, warning = FALSE}
#mean_mod1s_week<-emmeans(mod_Exp_meanDayNight_Week_s, ~DayNight * experiment + Week)
#mean_mod1s_week
```
...
```{r pairs_mod1s, message = FALSE, warning = FALSE}
#pairs(mean_mod1s_week, adjust=NULL)
```

Lets calculate the marginal mean for the model and make it a data frame which we can use to plot

```{r grid_mod1s, message = FALSE, warning = FALSE}
#gr_mod1s_week <- ref_grid(mod_Exp_meanDayNight_Week_s, cov.keep=c("DayNight","experiment","Week"))
#gr_emmeans_mod1s_week<-as.data.frame(emmeans(gr_mod1s_week, specs = c("DayNight","experiment","Week"), level=0.95))
```

Now, to define the information we will use to plot the data

```{r db_mod1s, message = FALSE, warning = FALSE}
#mean_mod1_weeks<-as.data.frame(mean_mod1s_week)
#raw_Exp_DayNight_MeanData <- mean_activity_DayNight[c("DayNight", "experiment", "DayNight_activity","Week")]
#plot_data_mod1s_week <- merge(gr_emmeans_mod1s_week, raw_Exp_DayNight_MeanData, by = c("DayNight", "experiment"))
```

And, finally plotting this model

```{r plot_mod1s, message = FALSE, warning = FALSE}
#| warning: false 
# ggplot(mean_mod1s_week, aes(x = DayNight, y = emmean, color = experiment)) +
#   geom_jitter(data = plot_data_mod1s_week, 
