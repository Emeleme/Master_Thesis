...
This is one of those code chunks were it seems that it is not necessary to do all here but if I don't do it nothing runs after
...
From that database let's delete the data of the capture day and the data that is on "No trial"
...
I also need to delete the data from the last day (day 7) that is after the releasing time (Sunset) because there were no birds in the cages and I was cleaning all night
...
Three Willow Warblers does not meet the criteria to be included into the analysis (They fall into the gray zone where northern and southern willow warblers cant be separated). So, I needed to delete them
...
```{r grid_mod1}
gr_mod1 <- ref_grid(mod_Exp_meanDayNight, cov.keep=c("DayNight","experiment"))
gr_emmeans_mod1<-as.data.frame(emmeans(gr_mod1, specs = c("DayNight","experiment"), 
                                       level=0.95))
```
...
```{r db_mod1}
mean_mod1<- as.data.frame(mean_mod1)
#Use the DayNight, experiment, and DayNigth_activity columns
...
plot_data_mod1 <- merge(gr_emmeans_mod1, raw_Exp_DayNight_MeanData,
...
```{r plot_mod1}
#| warning: false
ggplot(mean_mod1, aes(x = DayNight, y = emmean, color = experiment)) +
  geom_jitter(data = plot_data_mod1, aes(x = DayNight, y = DayNight_activity),
              color="lightgray") +
...
ggplot(mean_mod1s, aes(x = DayNight, y = emmean, color = experiment)) +
  geom_jitter(data = plot_data_mod1s, aes(x = DayNight, y = DayNight_activity),
              color="lightgray") +
  geom_point(position = position_dodge2(width = 2), size =1.5) +
  geom_linerange(aes(ymin = lower.CL, ymax = upper.CL), alpha = 0.5,
                 position = position_dodge2(width = 1), size=1)+
...
ggplot(mean_mod2s, aes(x = DayNight, y = emmean, color = experiment)) +
  geom_jitter(data = plot_data_mod2s, aes(x = DayNight, y = DayNight_activity), 
              color="lightgray") +
  geom_point(position = position_dodge2(width = 2), size =1.5) +
  geom_linerange(aes(ymin = lower.CL, ymax = upper.CL), alpha = 0.5,
                 position = position_dodge2(width = 1), size=1)+
...
ggplot(data_to_plot_w1, aes(x = time, y = date, fill = !!sym(activity))) +
  ...
  scale_fill_gradient(name = activity_alias, 
                      low = "lightcyan", high = "darkblue", na.value = "#FFFFFF") +
  theme_classic() +
  ...
  geom_vline(xintercept = sunrise_w1, color= "darkorange1", linewidth = 0.8)+
  geom_vline(xintercept = sunset_w1, color= "darkorange3", linewidth = 0.8)
```
...
ggplot(data_to_plot_w2, aes(x = time, y = date, fill = !!sym(activity))) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        axis.line = element_line(size = 0.5))+
  scale_x_discrete(breaks = c("02:10", "06:10", "10:10", "14:10", "18:10", "22:10"))+
  geom_vline(xintercept = sunrise_w2, color= "darkorange1", linewidth = 0.8)+
  geom_vline(xintercept = sunset_w2, color= "darkorange3", linewidth = 0.8)
```
...
ggplot(data_to_plot_w3, aes(x = time, y = date, fill = !!sym(activity))) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        axis.line = element_line(size = 0.5))+
  scale_x_discrete(breaks = c("02:10", "06:10", "10:10", "14:10", "18:10", "22:10"))+
  geom_vline(xintercept = sunrise_w3, color= "darkorange1", linewidth = 0.8)+
  geom_vline(xintercept = sunset_w3, color= "darkorange3", linewidth = 0.8)
```
...
ggplot(data_to_plot_w4, aes(x = time, y = date, fill = !!sym(activity))) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        axis.line = element_line(size = 0.5))+
  scale_x_discrete(breaks = c("02:10", "06:10", "10:10", "14:10", "18:10", "22:10"))+
  geom_vline(xintercept = sunrise_w4, color= "darkorange1", linewidth = 0.8)+
  geom_vline(xintercept = sunset_w4, color= "darkorange3", linewidth = 0.8)
...
ggplot(data_to_plot_w5, aes(x = time, y = date, fill = !!sym(activity))) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        axis.line = element_line(size = 0.5))+
  scale_x_discrete(breaks = c("02:10", "06:10", "10:10", "14:10", "18:10", "22:10"))+
  geom_vline(xintercept = sunrise_w5, color= "darkorange1", linewidth = 0.8)+
  geom_vline(xintercept = sunset_w5, color= "darkorange3", linewidth = 0.8)
```
